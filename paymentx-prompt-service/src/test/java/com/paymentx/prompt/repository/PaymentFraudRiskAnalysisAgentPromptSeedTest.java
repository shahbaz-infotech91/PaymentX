...
/**
 * Phase 4.5.3 - proves the real, seeded PAYMENT_FRAUD_RISK_ANALYSIS rows (V1_0_7 in
 * db.changelog-master.yaml) exist and are shaped exactly as designed, mirroring
 * PaymentDatabaseAnalysisAgentPromptSeedTest's own pattern.
 */
@Testcontainers
@DataJpaTest
@AutoConfigureTestDatabase(replace = AutoConfigureTestDatabase.Replace.NONE)
@Import(com.paymentx.prompt.config.JpaAuditingConfig.class)
class PaymentFraudRiskAnalysisAgentPromptSeedTest {

    private static final UUID TEMPLATE_ID = UUID.fromString("aaaaaaaa-9999-9999-9999-999999999999");
    ...
    void template_isSeededExactlyOnce_withTheRealPromptKey() {
        PromptTemplate template = promptTemplateRepository.findByPromptKey("PAYMENT_FRAUD_RISK_ANALYSIS").orElseThrow();
    ...
    @Test
    void neverAllowsFraudConfirmed_onlyPotentialRisk() {
        PromptVersion v1 = promptVersionRepository.findByPromptTemplateIdAndVersionNumber(TEMPLATE_ID, 1).orElseThrow();
        String content = v1.getContent();

        assertThat(content).contains("NEVER conclude \"fraud detected\" or \"fraud confirmed,\"");
        assertThat(content).contains("PaymentX has no mechanism capable of confirming fraud");
        assertThat(content).contains("never FRAUD_CONFIRMED, which is not a valid value for you to use");
    }

    @Test
    void forbidsAutomaticFraudConclusionFromAnySingleWeakSignal() {
        PromptVersion v1 = promptVersionRepository.findByPromptTemplateIdAndVersionNumber(TEMPLATE_ID, 1).orElseThrow();
        String content = v1.getContent();

        assertThat(content).contains("duplicate payment reference", "a payment retry", "a failed payment",
                "an amount-limit rejection", "a blacklist-related rejection reason", "a reconciliation mismatch",
                "an elevated audit-event frequency");
        assertThat(content).contains("may ever, by itself, justify more than a LOW risk indicator");
    }

    @Test
    void statesTheParticipantLookupLimitationExplicitly() {
        PromptVersion v1 = promptVersionRepository.findByPromptTemplateIdAndVersionNumber(TEMPLATE_ID, 1).orElseThrow();
        String content = v1.getContent();

        assertThat(content).contains("There is no participant-lookup tool available to you");
    }
    ...
    void forbidsInventedThresholds() {
        ...
        assertThat(content).contains("Never invent a numeric threshold");
        assertThat(content).contains("Never state a numeric percentage or statistical probability");
    }
